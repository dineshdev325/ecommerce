<div>
 <div class="flex justify-between px-10 pt-2 "><span class="text-xl ">{{$products->count()}} results for "<span class="font-bold">{{$query}}</span>"</span> </div>
    <div>
        @if ($products->count()>0)
    
        <div class="grid grid-cols-3 px-10 mt-1 product_container gap-9">
            ...
            <div class="py-3 product_item " x-data="{
            current_size:1,
            current_color:1,
            price:'',
            not_login:false,
            update(){
                ...
                <div class="z-30 -mt-10 isolate">
                    <div class="price_container p-2 flex items-center justify-between bg-[#1e2832] ">
                        <div class="font-bold text-white " x-text="'$ ' + price">
                            RS.1000
                        </div>
                        <div class="flex items-center gap-2 cursor-pointer" @auth
                            @click="livewire.emit('add_cart','{{$product->id}}',current_color,current_size)" @else
                            @click="not_login= !not_login" @endauth>
                                ...
                                <div class="flex items-center gap-2 color_list">
                                    ...
                                    <div class="p-1 mt-2 rounded-full cursor-pointer "
                                        ...
                                        <svg xmlns='http://www.w3.org/2000/svg' fill='none' viewBox='0 0 24 24'
                                            stroke-width='1.5' :stroke="current_color=={{$color_item->id}}? 'white':'none' "
                                            class='w-6 h-6'>
                ...
                <div x-show='not_login' class="w-full " x-data="{
                    ...
                    <p x-text="flash_message"
                        class="fixed inset-x-0 z-20 px-3 py-2 mx-auto text-white bg-black w-fit bottom-4"></p>
                </div>
            </div>
            @endforeach
            ...
            <div>
                ...
                }" x-show="show" x-cloak class="fixed inset-x-0 z-20 px-3 py-2 mx-auto text-white bg-black w-fit bottom-4">
                    {{session('cart_exist')}}</p>
            </div>
            @endif
        </div>
    
        @else
        <div class="px-10 text-xl pt-10 ">
 <img class="w-[25%] h-[25%] mx-auto" src="../noresult.svg" alt="">
 <h2 class="text-center pt-5">Not available at the moment</h2>
        </div>
